...
export class SongsService {
  ...
  //     .where("song.duration BETWEEN :minDuration AND :maxduration", { minDuration: "02:00", maxduration: "04:00" })
  //     .getMany();
  // }
  ...
  //   return this.songsRepository.createQueryBuilder("song").where("song.song_id IN (:...ids)", { ids }).getMany();
  // }

  // Get all songs and the count of songs

  // async getSongs() {
  //   const repsonse = await this.songsRepository.createQueryBuilder("song").getManyAndCount();

  //   return { songs: repsonse[0], count: response[1] };
  // }
  ...
  //     .where("song.album_id = :albumId", { albumId: 1 }).getRawOne();
